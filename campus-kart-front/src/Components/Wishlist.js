...
export default function Wishlist() {
  ...
  useLayoutEffect(() => {
    ...
    try {
      fetch(`https://campus-kart-api.ml/getProdfromWishlist/${id}`)
        .then((resp) => resp.json())
        .then((resp) => {
  ...
  const handleRemove = async (data) => {
    try {
      ...
      const resp = await fetch(
        `https://campus-kart-api.ml/deleteprodfromwishlist/${data.customerId}/${data.productId}`,
        requestOptions
      );
                  ...
                  <tr className="border-gray-200 border-b  ">
                    <th>
                      <img
                        ...
                        src={`https://campus-kart-api.ml/uploads/${data.imgId}`}
                        alt="shoe"
                      />
                    </th>
